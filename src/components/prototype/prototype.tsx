'use client';
import {nextCorridorPole} from '@/lib/prototype/decision-guard';
...
const [secondary,setSecondary]=useState<{scene:Scene;photoCount:number;failureCount:number;message:string;instructionReason:string}>({scene:'hold',photoCount:1,failureCount:0,message:'',instructionReason:''});
const [surface,setSurface]=useState<'field'|'office'>('field');
const [{scene,pole,message,instructionReason,photoCount,photoCount024,analysisReady024,evidenceDelivered024,evidenceFailureCount024,queuedEvidenceFailure024},dispatch]=useReducer(journeyReducer,undefined,()=>seedScene());
const resetSecondary=()=>{setActivePole('024');setSecondary({scene:'hold',photoCount:1,failureCount:0,message:'',instructionReason:''});};
...
<main className={styles.stage}><section className={styles.deviceStage} aria-label="Interactive IKE device"><DeviceView instructionReason={activePole==='025'?secondary.instructionReason:instructionReason} uploadPending={activePole==='024' && !evidenceDelivered024} scene={visibleScene} poleNumber={activePole==='024'?pole:activePole} expandedPole={activePole!=='024'} photoCount={activePole==='025'?secondary.photoCount:photoCount} feedback={activePole==='025'?secondary.message:message} onCapture={()=>dispatch({type:'capture'})} onFlag={()=>dispatch({type:'flag'})} onAddPhoto={()=>activePole==='025'?setSecondary(p=>({...p,photoCount:p.photoCount+1,scene:p.scene==='request'?'sent':p.scene,message:'Photo added to Pole 025. Work instruction unchanged.'})):dispatch({type:'add-photo'})} onNextPole={()=>setActivePole(nextCorridorPole(activePole))} onUnable={()=>activePole==='025'?setSecondary(p=>({...p,failureCount:p.failureCount+1,scene:'hold',message:'Office received: requested photos unavailable. Pole 025 remains on hold.'})):dispatch({type:'unable'})}/><p className={styles.deviceCaption}>IKE Field · Android capture concept</p></section></main>
</div>
<div className={styles.officeStage} id="panel-office" role="tabpanel" aria-labelledby="role-office" hidden={surface!=='office'} tabIndex={0}><OfficeView activePole={activePole} onPoleChange={setActivePole} secondaryPhotoCount={secondary.photoCount} secondaryFailureCount={secondary.failureCount} onSecondaryInstruction={(instruction,reason)=>setSecondary(p=>({...p,scene:instruction==='continue'?'continue':instruction==='request'?'request':instruction==='stopped'?'stopped':'hold',instructionReason:reason,message:`Supervisor instruction received: ${reason}`}))} evidenceFailureCount={evidenceFailureCount024} noFinding={selectedDemoScene==='unflagged'} key={resetKey} initialScene={selectedDemoScene} analysisReady={analysisReady024} evidenceDelivered={evidenceDelivered024} photoCount={photoCount024} onInstruction={(instruction,reason)=>dispatch({type:'instruction-received',instruction,reason})}/></div>
