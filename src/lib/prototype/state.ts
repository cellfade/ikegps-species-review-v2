...
export type JourneyState = { scene: Scene; pole: string; photoCount: number; photoCount024: number; localPhotoCount024: number; analysisReady024: boolean; evidenceDelivered024: boolean; message: string; instruction: 'await-review'|'hold'|'continue'; concernId: string; evidenceFailureCount024: number; queuedEvidenceFailure024: boolean };
...
export function journeyReducer(state: JourneyState, event: JourneyEvent): JourneyState {
  ...
  switch(event.type) {
    ...
    case 'unable': return {...state,scene:state.scene==='offline'?'offline':'hold',instruction:'hold',evidenceFailureCount024:state.evidenceFailureCount024+(state.scene==='offline'?0:1),queuedEvidenceFailure024:state.scene==='offline',message:state.scene==='offline'?'Unable to collect photos. Update saved on device; office delivery pending. Pole remains on hold.':'Office received: requested view could not be obtained. Pole remains on hold.'};
