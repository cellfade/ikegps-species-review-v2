...
export function seedScene(scene: Scene = 'finding'): JourneyState {
  return {scene, pole:'024', photoCount:1, photoCount024:['capture','offline'].includes(scene)?0:1, localPhotoCount024:1, analysisReady024:!['capture','analysis','offline'].includes(scene), evidenceDelivered024:!['capture','offline'].includes(scene), message:'', instruction:scene==='continue'?'continue':['hold','request','sent','offline'].includes(scene)?'hold':'await-review', concernId:'concern-024'};
}
...
export function journeyReducer(state: JourneyState, event: JourneyEvent): JourneyState {
  ...
  switch(event.type) {
    ...
    case 'instruction-received':
      ...
      return {...state,pole:'024',photoCount:state.localPhotoCount024,concernId:'concern-024',scene:event.instruction==='continue'?'continue':event.instruction==='request'?'request':'hold',instruction:event.instruction==='continue'?'continue':'hold',message:`Supervisor instruction received: ${event.reason}`};
  }
}
